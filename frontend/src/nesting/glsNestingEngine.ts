...
function allowed(part:NestingPart,settings:NestingSettings){return settings.rotations.filter(r=>(!part.lockDirection||r===0)&&(!part.allowedRotations||part.allowedRotations.includes(r)));}
...
export async function runGlsNesting(parts:NestingPart[],settings:NestingSettings,options:SearchOptions={}):Promise<SearchOutcome> {
  const start=performance.now(),budget=options.timeBudgetMs??settings.timeBudgetMs??10000;
  if(!Number.isFinite(budget)||budget<1||budget>300000)throw new Error('Süre bütçesi 1–300000 ms arasında olmalı.');
  validateSettings(settings);validateParts(parts);
  ...
  let iterations=0,penaltyUpdates=0,lastYield=start,lastPublish=-Infinity,phase='Başlangıç',stopped=false,phaseDeadline=Infinity;
  ...
  const keep=(positions:Position[])=>{
    ...
    if(compareLayouts(result,best,settings)<-1e-7){validateLayout(parts,settings,result);best=result;bestPositions=[...positions];publish();}
  };
  const find=async(instance:Instance,positions:Position[],sheet:number,ceiling=height,angles=allowed(instance.part,settings),quick=false):Promise<Position|undefined>=>{
    ...
    for(const rotation of angles){
      ...
      if(quick){
        const gap=settings.spacing+0.0002,xs=new Set([ifp.minX,ifp.maxX]),ys=new Set([ifp.minY,ifp.maxY]);
        for(const other of others){for(const x of [other.x,other.x+other.shape.width+gap,other.x-shape.width-gap])xs.add(x);for(const y of [other.y,other.y+other.shape.height+gap,other.y-shape.height-gap])ys.add(y);}
      ...
      for(const candidate of candidates){
        ...
        const extent=Math.max(...others.map(p=>p.y+p.shape.height),candidate.y+shape.height);
  ...
  best=resultOf(bestPositions);const validation=validateLayout(parts,settings,best);phase=options.signal?.aborted?'Durduruldu':stopped?'Süre doldu':'Tamamlandı';publish(true);
