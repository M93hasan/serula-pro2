...
export const DEFAULT_NESTING_SETTINGS: NestingSettings = {
  ...
  margin: 1, spacing: 0.3, rotations: [0], startCorner: 'bottom-right',
};
...
export function runNesting(parts: NestingPart[], settings: NestingSettings = DEFAULT_NESTING_SETTINGS, options: { preserveOrder?: boolean; searchStep?: number; candidateLimit?: number } = {}): NestingResult {
  ...
  const sortedParts = options.preserveOrder ? parts : [...parts].sort((a, b) => {
    const areaA = calculateAbsoluteArea(a.outerContour.points) - a.holes.reduce((sum, hole) => sum + calculateAbsoluteArea(hole.points), 0);
    const areaB = calculateAbsoluteArea(b.outerContour.points) - b.holes.reduce((sum, hole) => sum + calculateAbsoluteArea(hole.points), 0);
    return areaB - areaA;
  });
  const instances = createPartInstances(sortedParts, options.preserveOrder);
