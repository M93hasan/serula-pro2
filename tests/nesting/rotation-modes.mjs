...
const base = {...DEFAULT_NESTING_SETTINGS,sheetWidth:38,sheetHeight:38,margin:0,spacing:0,startCorner:'bottom-left'};
for (const [name, rotations, expected] of [
  ['0°',[0],false], ['0°–180°',[0,180],false], ['Any',ANY_ROTATIONS,true],
]) {
  ...
  if(expected) {
    ...
    assert(preview.every(p=>p.x>=-1e-7 && p.x<=38+1e-7 && p.y>=-1e-7 && p.y<=38+1e-7));
...
for(const [name,rotations] of [['0°',[0]],['0°–180°',[0,180]],['Any',ANY_ROTATIONS]]) {
...
const halfTurn=runNesting([part],{...base,sheetWidth:20,sheetHeight:50,rotations:[0,180]});
assert.equal(halfTurn.placedCount,0,"Half-turn mode must not rotate 90 degrees to fit");
